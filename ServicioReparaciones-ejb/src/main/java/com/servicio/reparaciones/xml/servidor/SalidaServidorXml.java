...
public class SalidaServidorXml implements IsalidaServidorXml, Serializable {
    ...
    public void generatedXML(String barcode, String url, String filename, Salida salida) {
        ...
        xmlSalida.setFechaSalidaArticulo(format.format(calendar.getTime()));
        xmlSalida.setBarcode(salida.getCode());
        xmlSalida.setConcepto(salida.getConcepto());
        xmlSalida.setNumeroFactura(salida.getNumeroFactura() + "");
        xmlSalida.setEntrega(
                salida.getUsername().getDatosPersonales().getNombres() + " "
                + salida.getUsername().getDatosPersonales().getApellidos());
        xmlSalida.setRecibe(
                salida.getQuienRecibe().getUsername().getDatosPersonales().getNombres() + " "
                + salida.getQuienRecibe().getUsername().getDatosPersonales().getApellidos());
        xmlSalida.setArticulo(
                salida.getArticulo().getNombre() + " - "
                + salida.getArticulo().getArtefacto() + " "
                + salida.getArticulo().getMarca() + " "
                + salida.getArticulo().getModelo() + " "
                + salida.getArticulo().getNumeroParte());
        xmlSalida.setBodega(
                salida.getBodega().getNombre() + " "
                + salida.getBodega().getProvincia());
        xmlSalida.setCantidad(salida.getCantidad().toString());
